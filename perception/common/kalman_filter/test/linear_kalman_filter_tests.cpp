...
TEST(LinearPredictResult, DefaultConstructor_GivenTypicalDimensions_ExpectSame)
{
    ...
    // Then
    EXPECT_EQ(unit.GetPredictedState().GetMean().rows(), kStateVectorDimension);
    EXPECT_EQ(unit.GetPredictedState().GetMean().cols(), 1);
    EXPECT_EQ(unit.GetPredictedState().GetCovariance().rows(), kStateVectorDimension);
    EXPECT_EQ(unit.GetPredictedState().GetCovariance().cols(), kStateVectorDimension);
}
...
TEST(LinearPredictResult, DefaultConstructor_GivenTypicalDimensions_ExpectInitialValues)
{
    ...
    // Then
    EXPECT_EQ(unit.GetPredictedState().GetMean(), ResultType::PredictedStateType::MeanType::Zero());
    EXPECT_EQ(unit.GetPredictedState().GetCovariance(), ResultType::PredictedStateType::CovarianceType::Zero());
}
...
TEST(LinearPredictResult, Constructor_GivenTypicalInputs_ExpectSame)
{
    // Given
    using ResultType = LinearPredictResult<double, kStateVectorDimension>;
    const auto mean = ResultType::PredictedStateType::MeanType::Identity();
    const auto covariance = ResultType::PredictedStateType::CovarianceType::Identity();

    // When
    const ResultType unit{mean, covariance};

    // Then
    EXPECT_EQ(unit.GetPredictedState().GetMean(), mean);
    EXPECT_EQ(unit.GetPredictedState().GetCovariance(), covariance);
}
...
TEST(LinearUpdateResult, DefaultConstructor_GivenTypicalDimensions_ExpectSame)
{
    ...
    // Then
    EXPECT_EQ(unit.GetUpdatedState().GetMean().rows(), kStateVectorDimension);
    EXPECT_EQ(unit.GetUpdatedState().GetMean().cols(), 1);
    EXPECT_EQ(unit.GetUpdatedState().GetCovariance().rows(), kStateVectorDimension);
    EXPECT_EQ(unit.GetUpdatedState().GetCovariance().cols(), kStateVectorDimension);
...
TEST(LinearUpdateResult, DefaultConstructor_GivenTypicalDimensions_ExpectInitialValues)
{
    ...
    // Then
    EXPECT_EQ(unit.GetUpdatedState().GetMean(), ResultType::UpdatedStateType::MeanType::Zero());
    EXPECT_EQ(unit.GetUpdatedState().GetCovariance(), ResultType::UpdatedStateType::CovarianceType::Zero());
...
TEST(LinearUpdateResult, Constructor_GivenTypicalInputs_ExpectSame)
{
    // Given
    using ResultType = LinearUpdateResult<double, kStateVectorDimension, kMeasurementVectorDimension>;
    const auto mean = ResultType::UpdatedStateType::MeanType::Identity();
    const auto covariance = ResultType::UpdatedStateType::CovarianceType::Identity();
    ...
    // Then
    EXPECT_EQ(unit.GetUpdatedState().GetMean(), mean);
    EXPECT_EQ(unit.GetUpdatedState().GetCovariance(), covariance);
...
TEST_F(LinearKalmanFilterFixture, LinearPrediction_GivenTypicalPredictModel_ExpectPrioriState)
{
    ...
    // Then
    const auto mean = priori_state.GetPredictedState().GetMean();
    const auto covariance = priori_state.GetPredictedState().GetCovariance();
...
TEST_F(LinearKalmanFilterFixture, LinearUpdate_GivenTypicalPrioriState_ExpectPosterioriState)
{
    ...
    // Then
    const auto mean = posteriori_state.GetUpdatedState().GetMean();
    const auto covariance = posteriori_state.GetUpdatedState().GetCovariance();
