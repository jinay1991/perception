...
///
#ifndef PERCEPTION_COMMON_LINEAR_KALMAN_FILTER_H
#define PERCEPTION_COMMON_LINEAR_KALMAN_FILTER_H
...
{
  public:
    using PrioriState = MultivariateMoments<T, kStateVectorDimension>;

    constexpr LinearPredictResult() : priori_state_{} {}

    explicit constexpr LinearPredictResult(const typename PrioriState::MeanType& mean,
                                           const typename PrioriState::CovarianceType& covariance)
        : priori_state_{mean, covariance}
    {
    }

    constexpr const PrioriState& GetPrioriState() const noexcept { return priori_state_; }

  private:
    PrioriState priori_state_;
};
...
{
  public:
    using PosterioriState = MultivariateMoments<T, kStateVectorDimension>;
    ...
    constexpr LinearUpdateResult()
        : posteriori_state_{}, kalman_gain_{GainType::Zero()}, estimated_measurement_{MeasurementType::Zero()}
    {
    }

    explicit constexpr LinearUpdateResult(const typename PosterioriState::MeanType& mean,
                                          const typename PosterioriState::CovarianceType& covariance,
                                          const GainType& kalman_gain,
                                          const MeasurementType& estimated_measurement)
        : posteriori_state_{mean, covariance}, kalman_gain_{kalman_gain}, estimated_measurement_{estimated_measurement}
    {
    }

    constexpr const PosterioriState& GetPosterioriState() const noexcept { return posteriori_state_; }
  ...
  private:
    PosterioriState posteriori_state_;
...
constexpr LinearPredictResult<T, kStateVectorDimension> LinearPrediction(
    ...
    const LinearPredictModel<T, kStateVectorDimension, kControlVectorDimension>& predict_model) noexcept
{
    return LinearPrediction(previous_state.GetPrioriState().GetMean(),
                            previous_state.GetPrioriState().GetCovariance(),
                            control_vector,
                            predict_model);
}
...
constexpr LinearPredictResult<T, kStateVectorDimension> LinearPrediction(
    ...
    const LinearPredictModel<T, kStateVectorDimension, kControlVectorDimension>& predict_model) noexcept
{
    return LinearPrediction(previous_state.GetPosterioriState().GetMean(),
                            previous_state.GetPosterioriState().GetCovariance(),
                            control_vector,
                            predict_model);
}
...
{
    const auto& x = priori_state.GetPrioriState().GetMean();
    const auto& P = priori_state.GetPrioriState().GetCovariance();
...
#endif  /// PERCEPTION_COMMON_LINEAR_KALMAN_FILTER_H
